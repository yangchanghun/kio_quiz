...
export const MainQuizResultPage = ({
  questions,
  userAnswers,
}: ResultPageProps) => {
...
const selectedOrders = q.choices
  .filter((c) => selectedIds.includes(c.id))
  .map((c) => c.order)
  .filter((order): order is number => order !== undefined);

  const isCorrect =
    selectedOrders.length === q.answers.length &&
    selectedOrders.every((order) => q.answers.includes(order));
    ...
    >
      {choice?.label}
            ...
            <div className="mb-3">
              ...
              <div className="flex flex-wrap gap-2">
{q.answers.map((order) => {
  const choice = q.choices.find((c) => c.order === order);
  return (
    <span
      key={order}
