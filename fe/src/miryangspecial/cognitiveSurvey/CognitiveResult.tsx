...
export default function CognitiveResult() {
  ...
  const level = score <= 5 ? "normal" : score <= 9 ? "warning" : "danger";

  const TEXT = {
    normal: "현재 인지기능은 정상 범위로 보입니다 ✅",
    warning: "경도 인지저하가 의심됩니다 ⚠️",
    danger: "인지장애 또는 치매가 강하게 의심됩니다 🚨",
  };
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center px-6 py-10">
      <h2 className="text-3xl font-bold mb-6">인지장애 검사 결과</h2>
      ...
      <div className="mt-8 max-w-xl text-lg space-y-4">
        {level !== "normal" && (
          <p>
            증상이 경미하더라도
            <strong className="text-yellow-300">조기 진단</strong>이 매우
            중요합니다.
          </p>
        )}
        <p>
          치매 상담 콜센터 ☎ <strong>1588-0678</strong>
        </p>
      </div>
